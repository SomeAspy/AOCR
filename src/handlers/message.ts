...
export async function handleMessage(message: Message) {
    if (
        message.author.bot ||
        !message.inGuild() ||
        (!config.ApplyToModerators && message.member?.permissions.has(PermissionsBitField.Flags.ManageGuild, true))
    ) {
        return;
    }
    ...
    if (config.CheckEmojis && message.content) {
        /<:.+?:[0-9]+?>/g.exec(message.content)?.forEach((emojiDeclaration) => {
            const emojiId = /:[0-9].+[0-9]>/.exec(emojiDeclaration)?.at(0)?.slice(1).slice(0, -1).toString();
            if (emojiId) {
                imagesToCheck.push(`https://cdn.discordapp.com/emojis/${emojiId}.webp?size=max&quality=lossless`);
            }
        });
    }
